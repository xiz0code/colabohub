...
@Service
public class PickupLabelPdfService {
    ...
    public byte[] generateLabel(PickupResponse pickup) {
        PdfBuilder pdf = new PdfBuilder();
        double pageWidth = mmToPoints(70d);
        double pageHeight = mmToPoints(70d);
        pdf.beginPage();
        drawSquareLabel(pdf, pickup, 0d, pageHeight, pageWidth, pageHeight);
        pdf.endPage(pageWidth, pageHeight);
        return pdf.build();
    }

    private void drawSquareLabel(PdfBuilder pdf, PickupResponse pickup, double x, double yTop, double width, double height) {
        double bottom = yTop - height;
        double centerX = x + (width / 2d);
        String title = "Retiro " + safeText(pickup.pickupNumber());
        String subtitle = safeText(pickup.storeName());
        String customerName = safeText(pickup.customerName());
        String description = safeText(pickup.description());
        String barcodeText = safeText(pickup.pickupBarcode());
        double inset = 8d;
        double barcodeWidth = width - (inset * 2d) - 18d;
        double barcodeHeight = 48d;
        double barcodeY = bottom + 34d;

        pdf.roundedRect(x + 3d, bottom + 3d, width - 6d, height - 6d, 10d, "1 1 1", "0.82 0.78 0.9");
        pdf.textCentered(centerX, bottom + 176d, 13.5d, truncate(title, 20), true);
        pdf.textCentered(centerX, bottom + 159d, 9.2d, truncate(subtitle, 24), false);
        if (!customerName.isBlank()) {
            pdf.textCentered(centerX, bottom + 144d, 8.5d, truncate(customerName, 22), false);
        }
        if (!description.isBlank()) {
            pdf.textCentered(centerX, bottom + 129d, 9d, truncate(description, 24), true);
        }
        drawCode128(pdf, barcodeText, x + ((width - barcodeWidth) / 2d), barcodeY, barcodeWidth, barcodeHeight);
        pdf.textCentered(centerX, bottom + 14d, 11.5d, barcodeText, true);
    }
    ...
    private void drawCode128(PdfBuilder pdf, String barcode, double x, double y, double maxWidth, double height) {
        ...
        double moduleWidth = Math.min(1.18d, maxWidth / totalModules);
    ...
    private List<Integer> encodeCode128(String barcode) {
        if (barcode == null || barcode.isBlank()) {
            throw new IllegalArgumentException("Pickup barcode content is required.");
        }
